...
If part of a manifest must keep a literal explicit placeholder for another tool, write `{{ !env.NAME }}`. kubenv will render that back to `{{ env.NAME }}` without substituting it, so you can mix kubenv-rendered fields with literal placeholders in the same manifest.
